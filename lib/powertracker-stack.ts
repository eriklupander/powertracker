...
export class PowertrackerStack extends cdk.Stack {
    constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
        ...
        const golangBuilder = new GolangBuilder(this, "go lambda builder");

        // Build PowerRecorder lambda that reads data from Tibber and stores in Timestream DB
        const powerRecorderFunction = golangBuilder.buildGolangLambda('powerRecorder', path.join(__dirname, '../functions/powerRecorder'), 10);
        ...
        const exporterLambdaFn = golangBuilder.buildGolangLambda('exporter-api', path.join(__dirname, '../functions/exporter'), 30);
        exporterLambdaFn.addToRolePolicy(timeStreamPolicy)

        // Create HTTP API Gateway in front of the lambda
        const apiGtw = new APIGatewayBuilder(this, "API GW builder")
            .createApiGatewayForLambda("exporter-api-endpoint", exporterLambdaFn, 'Powertracker endpoints')
